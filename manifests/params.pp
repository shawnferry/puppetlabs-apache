...
class apache::params inherits ::apache::version {
  ...
  if $::osfamily == 'RedHat' or $::operatingsystem =~ /^[Aa]mazon$/ {
    ...
    $_apache_version     = pick($apache::apache_version, '2.4')
    $_apache_ver_nodot   = delete($_apache_version, '.')

    $user                = 'webservd'
    $group               = 'webservd'
    $root_group          = 'root'
    $apache_name         = "web/server/apache-${_apache_ver_nodot}"
    $service_name        = "network/http:apache${_apache_ver_nodot}"
    $httpd_dir           = "/etc/apache2/${_apache_version}"
    $server_root         = "/usr/apache2/${_apache_version}"
    $scriptalias         = "/var/apache2/${_apache_version}/cgi-bin/"
    $error_log           = 'error_log'
    $access_log_file     = 'access_log'
    $conf_dir            = $httpd_dir
    $confd_dir           = "${httpd_dir}/conf.d"
    ...
    $conf_file           = 'httpd.conf'
    $ports_file          = "${conf_dir}/ports.conf"
    $logroot             = "/var/apache2/${_apache_version}/logs"
    $logroot_mode        = undef
    $lib_path            = "/usr/apache2/${_apache_version}/libexec"
    $mpm_module          = 'prefork'
    ...
    $suphp_configpath    = undef
    $mod_packages        = {
      'dbd'           => "/server/apache-${_apache_ver_nodot}/module/apache-dbd",
      'dtrace'        => "/server/apache-${_apache_ver_nodot}/module/apache-dtrace",
      'fcgid'         => "/server/apache-${_apache_ver_nodot}/module/apache-fcgid",
      'gss'           => "/server/apache-${_apache_ver_nodot}/module/apache-gss",
      'jk'            => "/server/apache-${_apache_ver_nodot}/module/apache-jk",
      'ldap'          => "/server/apache-${_apache_ver_nodot}/module/apache-ldap",
      'lua'           => "/server/apache-${_apache_ver_nodot}/module/apache-lua",
      'security'      => "/server/apache-${_apache_ver_nodot}/module/apache-security",
      'ssl-fips-140'  => "/server/apache-${_apache_ver_nodot}/module/apache-ssl-fips-140",
      'ssl'           => "/server/apache-${_apache_ver_nodot}/module/apache-ssl",
      'wsgi-26'       => "/server/apache-${_apache_ver_nodot}/module/apache-wsgi-26",
      'wsgi-27'       => "/server/apache-${_apache_ver_nodot}/module/apache-wsgi-27",
      'wsgi-34'       => "/server/apache-${_apache_ver_nodot}/module/apache-wsgi-34",
    }
    $session_cache          = "/var/run/apache2/${_apache_version}/ssl_scache(512000)"
    $stapling_cache         = "/var/run/apache2/${_apache_version}/ssl_stapling(32768)"
    $mod_libs               = {
      'php5' => 'libphp5.so',
    }
    $conf_template          = 'apache/httpd.conf.erb'
    ...
    $keepalive_timeout      = 15
    $max_keepalive_requests = 100
    $fastcgi_lib_path       = "/var/apache2/${_apache_version}/htdocs/fcgid"
    $mime_support_package   = 'system/core-os'
    $mime_types_config      = '/etc/mime.types'
    $docroot                = "/var/apache2/${_apache_version}/htdocs"
    $cas_cookie_path        = undef
    $modsec_crs_package     = undef
    $modsec_crs_path        = undef
    $modsec_dir             = undef
    $modsec_default_rules   = undef
    $error_documents_path   = "/var/apache2/${_apache_version}/error"
    $wsgi_socket_prefix     = undef
    $alias_icons_path       = "/var/apache2/${_apache_version}/icons"
    $pidfile                = "/system/volatile/apache2/${_apache_version}/httpd.pid"
